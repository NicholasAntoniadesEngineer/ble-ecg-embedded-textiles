...
unsigned char offsets_applied = 0;
...
eeprom_t eeData[EEPROM_LENGTH] = {0x0002, 0x030F, 0x0000, 0,0, 0,0x4000,0, 0,0,0x4000, 0,0,0,8};
...
void setupPIC()
{
    ...
    LATB=1; TRISB=0x4004;   //B14 SDI     B2 URX        (B0 high LED OFF)
             ...
             if (isValidASCIIHEX(c))
             {
                ...
                if (protocolParamCount==5)
                {
                    ...
                    if (candidateUID==boardSettings.uniqueID)
                    {
                        ...
                        if (boardSettings.constellationID == 1) doPacing=0; //reenable pacing
	...
	if (rxHead!=rxTail)
	{
        ...
        if (blinders_on == 0)
        {
            ...
            if (dummy=='(')
            {
                ...
                WriteEEPROM(IMU_EEPROM_LENGTH,boardSettings.asUInts[IMU_EEPROM_LENGTH]);            //update eeprom
                LEDblink();
            }
...
{
    BMI160_setAccelOffsetEnabled(1);
    BMI160_setXAccelOffset(boardSettings.accX);
    BMI160_setYAccelOffset(boardSettings.accY);
    BMI160_setZAccelOffset(boardSettings.accZ);
    BMI160_setXGyroOffset(boardSettings.gyrX);
    BMI160_setYGyroOffset(boardSettings.gyrY);
    BMI160_setZGyroOffset(boardSettings.gyrZ);
}
