...
int  I2C1_M_Read(uint8_t, uint8_t, int, uint8_t *);
int  I2C1_M_ReStart(void);
int  I2C1_M_SendByte(uint8_t);
int  I2C1_M_Start(void);
int  I2C1_M_Stop(void);
void I2C1_M_Write(uint8_t, uint8_t, int, uint8_t *);
//----------------Function Prototypes-------------------
...
int I2C1_M_BusReset()
{
	...
	LATBbits.LATB8 = 1;										//PORTBbits.RB8 = 1 is equivalent
	LATBbits.LATB9 = 1;

	__delay_us(10);											//Need 5uS delay
	if(PORTBbits.RB8 == 0)									//Read if line actually went high
	{
	...
	while(i>0)
	{
		if(PORTBbits.RB9 == 1)								//If SDA is high, then we are done
		{
			break;
		}
	...
	if((PORTB & 0x0300) != 0x0300)							//We are ok if SCL and SDA high
	{
		return 2;
	}
...
int  I2C1_M_Poll(uint8_t DevAddr)
{
	...
	if(I2C1_M_Start() == 0)
	{
		retval = I2C1_M_SendByte((uint8_t)SlaveAddr);
...
//0x0000 Success
//0x8001 Error
int  I2C1_M_Read(uint8_t DevAddr, uint8_t SubAddr, int ByteCnt, uint8_t *buffer)
{
	uint8_t SlaveAddr;
	int retval;
	int i;
	...
	if( I2C1_M_SendByte((uint8_t)SlaveAddr) != 0)				//Slave Addr
	{
		I2C1_M_Stop();
		SetI2C1BusDirty;
		return 0x8001;
	...
	if( I2C1_M_SendByte((uint8_t)SubAddr) != 0)				//Sub Addr
	{
		I2C1_M_Stop();
		SetI2C1BusDirty;
		return 0x8001;
...
int I2C1_M_SendByte(uint8_t cData)
{
	int t;
...
void I2C1_M_Write(uint8_t DevAddr, uint8_t SubAddr, int ByteCnt, uint8_t *buffer)
{
	int i;
	uint8_t SlaveAddr;
